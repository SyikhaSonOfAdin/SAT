...
class CheckOut {
    ...
    addWC = async (CONNECTION, worker_id, date, time) => {
        const QUERY = [
            `INSERT INTO ${TABLES.WORKER_CHECKOUT.TABLE} (WORKER_ID, DATE, TIME, SHIFT) VALUES (?, ?, ?, COALESCE((SELECT CI.${TABLES.LIST_WORKER.COLUMN.SHIFT} FROM ${TABLES.LIST_WORKER.TABLE} AS CI WHERE CI.${TABLES.LIST_WORKER.COLUMN.ID} = ?), 0 )) ON DUPLICATE KEY UPDATE TIME = IF(VALUES(TIME) > TIME, VALUES(TIME), TIME);`
        ]
        const PARAMS = [[worker_id, date, time, worker_id]]
    ...
    cleanUp = async (CONNECTION) => {
        ...
        try {
            for (let i = 0; i < QUERY.length; i++) {
                console.time(`Query CheckOut ${i}`)
                await CONNECTION.query(QUERY[i])
                console.timeEnd(`Query CheckOut ${i}`)
            }
        } catch (error) {
            throw error
        }
